...
**Stage.size**:
当前终端 (cols, rows)，固定 240×40，ioctl 仅用于 fallback。
...
**RenderParams**:
预设参数包（mode + edgeThreshold + bwThreshold）。每条 static let 是一份预设：.outline（edge 60）/ .filled / .bw（bw 128）/ .bwDither（bw 128）/ .bwEdge（edge 200）。CatAct 持有一份，CLI 旗标经 resolveParams() 合成。
...
**Stage.fixedSize**:
固定终端尺寸 240×40。Stage.enter() 发送 xterm resize 序列 `\x1b[8;40;240t`，所有渲染决策基于此尺寸。
